...
out = "build/spacetrader-arm.jar" if ARM_ONLY else "build/spacetrader.jar"
...
seen = set()
def add(z, name, data):
    if ARM_ONLY and name.startswith(SKIP): return
...
with zipfile.ZipFile(out, "w", zipfile.ZIP_DEFLATED) as z:
    ...
    for jar in sorted(glob.glob("libs/*.jar")):
        ...
        with zipfile.ZipFile(jar) as j:
            for n in j.namelist():
                if ARM_ONLY and n.endswith((".dylib", ".dll")): continue  # macOS/Windows natives are useless on the device
                add(z, n, j.read(n))
